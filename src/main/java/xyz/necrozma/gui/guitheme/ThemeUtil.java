...
public final class ThemeUtil implements InstanceAccess {
    ...
    public Color getThemeColor(float colorOffset, final ThemeType type, final float timeMultiplier) {
        if (timer.hasReached(50 * 5)) {
            timer.reset();
            theme = ((ModeSetting) Objects.requireNonNull(Client.INSTANCE.getMM().getSetting("Interface", "Theme"))).getMode();
            color = new Color(Client.INSTANCE.CLIENT_THEME_COLOR);
        }
        ...
        switch (type) {
            ...
            case ARRAYLIST:
                switch (theme) {
                    ...
                    case "Never Lose":
                        ...
                        color = ColorUtil.liveColorBrighter(Client.INSTANCE.CLIENT_THEME_COLOR_BRIGHT_COLOR, offset1);
                        break;
            ...
            case LOGO:
                switch (theme) {
                    ...
                    case "Comfort":
                    case "Minecraft":
                        color = new Color(Client.INSTANCE.CLIENT_THEME_COLOR_BRIGHT);
                        break;
