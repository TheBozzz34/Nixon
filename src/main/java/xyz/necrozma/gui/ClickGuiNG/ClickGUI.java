package xyz.necrozma.gui.ClickGuiNG;
...
//    private ScriptState scriptState = ScriptState.NONE;
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
        ...
        final boolean blur = ((BooleanSetting) Objects.requireNonNull(Client.INSTANCE.getMM().getSetting("ClickGui", "Blur Background"))).isEnabled();
        ...
        final boolean canPopUp = Objects.requireNonNull(Client.INSTANCE.getMM().getModuleFromString("PopOutAnimation")).isToggled() && PopOutAnimation.clickGuiValue;
        ...
        final ModeSetting theme = (ModeSetting) Client.INSTANCE.getMM().getSetting("ClickGui", "Theme");
        ...
        if (((BooleanSetting) Objects.requireNonNull(Client.INSTANCE.getMM().getSetting("ClickGui", "Transparency"))).isEnabled()) {
                ...
                for (final Module m : Client.INSTANCE.getMM().getModules().values()) {
                    ...
                    if (c == selectedCat) {
                        ...
                        if (c != Category.CONFIGS && c != Category.STATISTICS) {
                            ...
                            if (m.isToggled()) updateRainbow(theme);
                        }
                    }
                }
                break;
        ...
        if (selectedCat == Category.STATISTICS) {
            ...
            final long milliseconds = System.currentTimeMillis() - Client.timeJoinedServer;
...
//                y = sr.getScaledHeight() / 2;
            ...
            ((NumberSetting) Objects.requireNonNull(Client.INSTANCE.getMM().getSetting("Radar", "Radar X"))).setValue(x);
            ((NumberSetting) Objects.requireNonNull(Client.INSTANCE.getMM().getSetting("Radar", "Radar Y"))).setValue(y);
        }
...
//            }
        ...
        final int radarX = (int) ((NumberSetting) Objects.requireNonNull(Client.INSTANCE.getMM().getSetting("Radar", "Radar X"))).getValue();
        final int radarY = (int) ((NumberSetting) Objects.requireNonNull(Client.INSTANCE.getMM().getSetting("Radar", "Radar Y"))).getValue();
