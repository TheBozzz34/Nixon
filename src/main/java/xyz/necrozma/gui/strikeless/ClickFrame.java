...
public class ClickFrame implements ClickGUIType, InstanceAccess {
    ...
    public ClickFrame(final Category category, final float frameX, final float frameY) {
        this.modules = Client.INSTANCE.getMM().getModulesByCategory(category);
    ...
    public void draw(final StrikeGUI parent, final int mouseX, final int mouseY) {
        ...
        switch (Objects.requireNonNull(theme).getMode()) {
            ...
            case "Disco":
                theme.cycle(true);
                Client.INSTANCE.getNM().registerNotification("Disco Theme doesn't work with Dropdown ClickGui.");
                break;
        }
        ...
        if (expanded) {
            ...
            if (category == Category.CONFIGS) {
                ...
                for (final Module module : modules) {
                    ...
                    final Color backgroundColor = module.isToggled()
                    ...
                    comfortaaNormal.drawString(module.getName(), frameX + 4.0F, titleY, Color.WHITE.getRGB());

                    // Draw the settings
                    final List<Settings> settings = module.getSettings();
                    if (settings.size() > 0) {
        ...
        if (expanded) {
            ...
            for (final Module module : modules) {
                ...
                if (expandedModuleIndices.contains(module)) {
                    for (final Settings setting : module.getSettings()) {
                        if (setting.isHidden()) continue;
