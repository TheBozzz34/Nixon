    ...
    public void onEnable() {
        ...
        switch (mode.getMode()) {
            case "Hypixel New":
                ...
                if (!firstEnable) {
                    firstEnable = true;
                    Client.INSTANCE.getNM().registerNotification("You need speed effect to make the fly work.");
                }
                break;
            case "Redesky":
                if (!firstEnable) {
                    firstEnable = true;
                    Client.INSTANCE.getNM().registerNotification("Credits to Dort for the fly.");
                }
                break;
            ...
            case "Vulcan":
                if (inf.isEnabled() && !firstEnable) {
                    firstEnable = true;
                    Client.INSTANCE.getNM().registerNotification("Having inf on might flag.");
                }
                break;
        ...
        switch (mode.getMode()) {
            ...
            case "Verus": {
                if (ticks == 1) {
                    if (mc.thePlayer.isPotionActive(Potion.jump)) {
                        Client.INSTANCE.getNM().registerNotification(("Cannot enable " + this.getName() + " with Jump Boost."));
                        toggle();
                        return;
                    }

                    if (!mc.thePlayer.onGround) {
                        Client.INSTANCE.getNM().registerNotification(("Cannot enable " + this.getName() + " in air."));
                        toggle();
                        return;
                    }
                }
            ...
            case "Minemenclub": {
                if (MoveUtil.isMoving() && (PlayerUtil.isOnServer("minemen") || PlayerUtil.isOnServer("mineman"))) {
                    ChatUtil.sendMessage("made by billionairkse111");
                    if (mc.thePlayer.onGround) {
                        if (!beingDmged) {
    ...
    public void onPacketReceive(final PacketReceiveEvent event) {
        ...
        if (p instanceof S08PacketPlayerPosLook) {
            ...
            switch (mode.getMode()) {
                ...
                case "Verus":
                    this.toggle();
                    Client.INSTANCE.getNM().registerNotification(("Disabled " + this.getName() + " due to flag."));
                    break;
