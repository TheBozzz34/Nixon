...
import xyz.necrozma.gui.guitheme.ThemeType;
...
import xyz.necrozma.module.impl.motion.Speed;
...
public final class Aura extends Module {
    ...
    public void onPreMotion(final PreMotionEvent event) {
        ...
        //targetstrafe = this.getModule(TargetStrafe.class).isEnabled();
        targetstrafe = false;
...

        {
            ...
            if (this.silentRotations.isEnabled() && !rotationMode.is("None")) {
                if (!(PlayerUtil.isOnServer("Hypixel") && (Client.INSTANCE.getMM().getModule(Speed.class).isToggled() /*|| Client.INSTANCE.getMM().getModule(LongJump.class).isEnabled()*/))) {
    ...
    public void onUpdate(final EventUpdate event) {
        ...
        /*
        if ((Client.INSTANCE.getMM().getModule(Scaffold.class).isEnabled() && !attackWithScaffold.isEnabled())
        ...
        if ((!newSwing.isEnabled() && attack) || (newSwing.isEnabled() && this.hitTicks > delayValue)) {
            ...
            if (extendedRangeBool.isEnabled()) {
                ...
                while (distance > (range.getValue() - 0.5657) * 6.5 && packets < 100) {
                    ...
                    ChatUtil.sendMessage("Distance: " + MathHelper.sqrt_double(v * v + v1 * v1));
            ...
            switch (mode.getMode()) {
                case "Single": {
                    ...
                    if (this.keepSprint.isEnabled() && (!mc.thePlayer.onGround || !onlyInAir.isEnabled())) {
                        ...
                        Client.INSTANCE.getNM().registerNotification("Attacking " + target + " with C02 packet.");
                    } else {
                        mc.playerController.attackEntity(mc.thePlayer, target);
                        Client.INSTANCE.getNM().registerNotification("Attacking " + target + " with player controller.");
                    }
    ...
    public void onRender2DEvent(final Render2DEvent event) {
        ...
        if (rotationMode.is("Custom") || rotationMode.is("Custom Simple") || rotationMode.is("Custom Advanced")) {
            ...
            comfortaa.drawString("Last Yaw: " + lastYaw, 1, 36, -1);
            comfortaa.drawString("Last Pitch: " + lastPitch, 1, 48, -1);
            comfortaa.drawString("Server Yaw: " + serverYaw, 1, 60, -1);
            comfortaa.drawString("Server Pitch: " + serverPitch, 1, 72, -1);
        }

        if (target != null) {
            comfortaa.drawString("Target: " + target, 1, 84, -1);
            comfortaa.drawString("Target Pos: " + targetPosX + ", " + targetPosY + ", " + targetPosZ, 1, 96, -1);
            comfortaa.drawString("Target Distance: " + mc.thePlayer.getDistanceToEntity(target), 1, 108, -1);
        } else {
            comfortaa.drawString("Target: None", 1, 84, -1);
        }

        if (positionOnPlayer != null) {
            comfortaa.drawString("Position On Player: " + positionOnPlayer.xCoord + ", " + positionOnPlayer.yCoord + ", " + positionOnPlayer.zCoord, 1, 120, -1);
        } else {
            comfortaa.drawString("Position On Player: None", 1, 120, -1);
        }

        if (lastPositionOnPlayer != null) {
            comfortaa.drawString("Last Position On Player: " + lastPositionOnPlayer.xCoord + ", " + lastPositionOnPlayer.yCoord + ", " + lastPositionOnPlayer.zCoord, 1, 132, -1);
        } else {
            comfortaa.drawString("Last Position On Player: None", 1, 132, -1);
        }
        comfortaa.drawString("CPS: " + cps, 1, 144, -1);
        comfortaa.drawString("Hit Ticks: " + hitTicks, 1, 156, -1);
        comfortaa.drawString("Target Index: " + targetIndex, 1, 168, -1);
        comfortaa.drawString("Blocking: " + blocking, 1, 180, -1);
        comfortaa.drawString("Sin Wave Ticks: " + sinWaveTicks, 1, 192, -1);
        comfortaa.drawString("Range: " + range.getValue(), 1, 204, -1);
        comfortaa.drawString("Extended Range Numb: " + extendedRangeNumb.getValue(), 1, 216, -1);
        comfortaa.drawString("Extended Range Bool: " + extendedRangeBool.isEnabled(), 1, 228, -1);
        comfortaa.drawString("Ray Trace: " + rayTrace.isEnabled(), 1, 240, -1);
        comfortaa.drawString("Through Walls: " + throughWalls.isEnabled(), 1, 252, -1);
        comfortaa.drawString("Silent Rotations: " + silentRotations.isEnabled(), 1, 264, -1);
        comfortaa.drawString("Keep Sprint: " + keepSprint.isEnabled(), 1, 276, -1);
        comfortaa.drawString("Only In Air: " + onlyInAir.isEnabled(), 1, 288, -1);
        comfortaa.drawString("Strafe: " + strafe.isEnabled(), 1, 300, -1);
        comfortaa.drawString("New Combat: " + newCombat.isEnabled(), 1, 312, -1);
        comfortaa.drawString("New Swing: " + newSwing.isEnabled(), 1, 324, -1);
        comfortaa.drawString("Rotation Mode: " + rotationMode.getMode(), 1, 336, -1);
        comfortaa.drawString("Block Mode: " + blockMode.getMode(), 1, 348, -1);
        comfortaa.drawString("Sorting Mode: " + sortingMode.getMode(), 1, 360, -1);
        comfortaa.drawString("Predict: " + predict.getValue(), 1, 372, -1);
        comfortaa.drawString("Random: " + random.getValue(), 1, 384, -1);
        comfortaa.drawString("Max Rotation: " + maxRotation.getValue(), 1, 396, -1);
        comfortaa.drawString("Min Rotation: " + minRotation.getValue(), 1, 408, -1);
        comfortaa.drawString("Min Yaw Rotation: " + minYawRotation.getValue(), 1, 420, -1);
        comfortaa.drawString("Max Yaw Rotation: " + maxYawRotation.getValue(), 1, 432, -1);
        comfortaa.drawString("Min Pitch Rotation: " + minPitchRotation.getValue(), 1, 444, -1);
        comfortaa.drawString("Max Pitch Rotation: " + maxPitchRotation.getValue(), 1, 456, -1);
        comfortaa.drawString("Sin Wave Speed: " + sinWaveSpeed.getValue(), 1, 468, -1);
        comfortaa.drawString("Derp Speed: " + derpSpeed.getValue(), 1, 480, -1);
        comfortaa.drawString("Predict Position: " + predictedPosition.isEnabled(), 1, 492, -1);
        comfortaa.drawString("Attack With Scaffold: " + attackWithScaffold.isEnabled(), 1, 504, -1);
        comfortaa.drawString("Attack In Interfaces: " + attackInInterfaces.isEnabled(), 1, 516, -1);
        comfortaa.drawString("On Click: " + onClick.isEnabled(), 1, 528, -1);
        comfortaa.drawString("Mode: " + mode.getMode(), 1, 540, -1);
        comfortaa.drawString("Target ESP: " + targetESP.isEnabled(), 1, 552, -1);
        comfortaa.drawString("Display Range: " + displayRange.isEnabled(), 1, 564, -1);
        comfortaa.drawString("Target On Player: " + targetOnPlayer.isEnabled(), 1, 576, -1);
        comfortaa.drawString("Disable On World Change: " + disableOnWorldChange.isEnabled(), 1, 588, -1);
        comfortaa.drawString("Min CPS: " + minCps.getValue(), 1, 600, -1);
        comfortaa.drawString("Max CPS: " + maxCps.getValue(), 1, 612, -1);
        comfortaa.drawString("Max Targets: " + maxTargets.getValue(), 1, 624, -1);
        comfortaa.drawString("Predict: " + predict.getValue(), 1, 636, -1);
        comfortaa.drawString("Random: " + random.getValue(), 1, 648, -1);
        comfortaa.drawString("Extended Range Numb: " + extendedRangeNumb.getValue(), 1, 660, -1);
    ...
    public void onRender3DEvent(final Render3DEvent event) {
        ...
        if (this.targetESP.isEnabled()) {
            switch (mode.getMode()) {
                ...
                case "Switch": {
                    if (target != null) {
                        drawCircle(target, 0.67, Client.CLIENT_THEME_COLOR, true);
                    }
                    break;
                }
                ...
                case "Multi": {
                    for (final EntityLivingBase entity : getTargets()) {
                        drawCircle(entity, 0.67, Client.CLIENT_THEME_COLOR, true);
                    }
                    break;
                }
            }
     ...
     */
    private void update() {
        if ((/*Client.INSTANCE.getMM().getModule(Scaffold.class).isEnabled() && !attackWithScaffold.isEnabled()) || */((mc.currentScreen != null && !(mc.currentScreen instanceof ClickGUI)) && !attackInInterfaces.isEnabled()))) {
            unblock();
            target = null;
            return;
        }
    ...
    private void drawCircle(final Entity entity, final double rad) {
        ...
        for (int i = 0; i <= 90; ++i) {
            RenderUtil.color(new Color(Client.INSTANCE.CLIENT_THEME_COLOR));
