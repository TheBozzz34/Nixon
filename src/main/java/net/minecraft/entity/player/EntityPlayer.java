...
public abstract class EntityPlayer extends EntityLivingBase implements InstanceAccess {
    /**
    ...
    // public EvictingList<Vec3> previousPositions = new EvictingList<>(15);
     ...
     * Args: itemstack, flag
     */
    public EntityItem dropPlayerItemWithRandomChoice(final ItemStack itemStackIn) {
    ...
    public EntityPlayer.EnumStatus trySleep(final BlockPos bedLocation) {
        if (!this.worldObj.isRemote) {
            ...
            final List<EntityMob> list = this.worldObj.<EntityMob>getEntitiesWithinAABB(EntityMob.class, new AxisAlignedBB((double) bedLocation.getX() - d0, (double) bedLocation.getY() - d1, (double) bedLocation.getZ() - d0, (double) bedLocation.getX() + d0, (double) bedLocation.getY() + d1, (double) bedLocation.getZ() + d0));
    ...
    public float getAIMoveSpeed() {
        if (Objects.requireNonNull(Rise.INSTANCE.getModuleManager().getModule("Scaffold")).isEnabled() && ((BooleanSetting) Objects.requireNonNull(Rise.INSTANCE.getModuleManager().getSetting("Scaffold", "Ignore Speed"))).isEnabled()) {
